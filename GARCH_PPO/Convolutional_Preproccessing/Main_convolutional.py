...
from PPO_Agent_Convolutional import Agent
from PPO_Agent_Convolutional import Preproccess
import numpy as np
...
def train(n_episodes=500, commission_rate=.0025, reward_type='standard', ticker='.INX'):
    shutup.please()
    data = GetData(convolutional=True, recurrent=False, ticker=ticker)
    agent = Agent()
    ...
    for i in tqdm(range(n_episodes), desc=f'{reward_type}'):
        ...
        while not done:
            ...
            action, prob, val, minutely_data, daily_data, weekly_data = \
                agent.choose_action(minutely_data, daily_data, weekly_data)
            cntr += 1
